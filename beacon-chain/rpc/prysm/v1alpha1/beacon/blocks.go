...
func convertToBlockContainer(blk interfaces.SignedBeaconBlock, root [32]byte, isCanonical bool) (*ethpb.BeaconBlockContainer, error) {
	...
	case version.EIP4844:
		if blk.IsBlinded() {
			rBlk, err := blk.PbBlinded4844Block()
			if err != nil {
				return nil, err
			}
			ctr.Block = &ethpb.BeaconBlockContainer_BlindedEip4844{BlindedEip4844: rBlk}
		} else {
			rBlk, err := blk.Pb4844Block()
			if err != nil {
				return nil, err
			}
			ctr.Block = &ethpb.BeaconBlockContainer_Eip4844{Eip4844: rBlk}
		}
	default:
